...
export const Navbar = () => {
  ...
  return (
    <div className='rounded-div flex items-center justify-between h-20 font-bold'>
        <Link to='/'>
            <h1 className='text-2xl'>Barac Crypto Viewer</h1>
        </Link>
        ...
        <div className={nav ? 'md:hidden fixed left-0 top-20 flex flex-col items-center justify-between w-full h-[90%] bg-primary ease-in duration-300 z-10'
         : 'fixed left-[-100%] top-20 h-[90%] flex flex-col items-center justify-between ease-in duration-300' }>
            ...
            <div className='flex flex-col w-full p-4'>
                <Link to='signin'>
                    <button className='w-full my-2 p-3 bg-primary text-primary border border-secundary rounded-2xl shadow-xl'>Sign In</button>
                </Link>
            </div>
            <div>
                <Link to='signup'>
